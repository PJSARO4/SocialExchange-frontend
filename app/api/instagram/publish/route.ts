...
async function checkContainerStatus(containerId: string, accessToken: string): Promise<string> {
  const url = `https://graph.instagram.com/v21.0/${containerId}?fields=status_code&access_token=${accessToken}`;
...
async function publishMedia(containerId: string, instagramUserId: string, accessToken: string): Promise<string> {
  const url = `https://graph.instagram.com/v21.0/${instagramUserId}/media_publish`;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const url = `https://graph.instagram.com/v21.0/${instagramUserId}/content_publishing_limit?fields=quota_usage,config&access_token=${accessToken}`;
